...
cmd({
  ...
  if (!isOwner) {
    return reply("❌ You Are Not The SHANUWA-MD Main Owner !");
  }
  if (!quotedMessage) {
...
cmd({
  ...
  if (!isOwner) {
    return replyMsg("❌ You Are Not The SHANUWA-MD Main Owner !");
  }
  if (!quotedMsg) {
...
function startAutoBio(conn) {
    ...
    autoBioInterval = setInterval(async () => {
        ...
        const bioText = `👨‍💻 SHANUWA-MD [${time}] 👨‍💻`;  // Set the bio text with time
