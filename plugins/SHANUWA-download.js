...
╚═══════════════════════❐
    ...
    bot.ev.on("messages.upsert", async (update) => {
      ...
      if (isReply) {
        ...
        switch (userChoice) {
          case "1":
            await bot.sendMessage(from, { video: { url: links.SD }, caption: ">*SHANUWA-MD SD video." });
            break;
          case "2":
            await bot.sendMessage(from, { video: { url: links.HD }, caption: ">*SHANUWA-MD HD video." });
            break;
          case "3":
...
╚═══════════════════════❐
    ...
    bot.ev.on("messages.upsert", async (update) => {
      ...
      if (isReply) {
        ...
        if (userChoice === '1') {
          await bot.sendMessage(from, { video: { url: dl_link.download_mp4_1 }, caption: "> SHANUWA-MD SD video!" });
        } else if (userChoice === '2') {
          await bot.sendMessage(from, { video: { url: dl_link.download_mp4_2 }, caption: "> SHANUWA-MD HD video!" });
...
try{
 ...
 await conn.sendMessage(from, { text: '📥 *ꜱʜᴀɴᴜᴡᴀ-ᴍᴅ ιѕ ᴅᴏᴡɴʟᴏᴅɪɴɢ...* 📥' }, { quoted: mek });
  ...
  await m.react('✅');
  let video = data.url;
  let dev = '© 2024 SHANUWA-MD FB DOWNLOAD HD.'
...
cmd({
  ...
  try {
    ...
    await bot.sendMessage(from, {
      ...
      mimetype: "video/mp4",
      caption: "> *© ‍ᴘᴏᴡᴇʀᴅ ʙʏ ꜱʜᴀɴᴜᴡᴀ ᴍᴅ ᴠ4
    }, { quoted: message });
  } catch (error) {
